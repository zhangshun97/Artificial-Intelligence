...
def create_nqueens_csp(n=8):
    ...
    for var1 in range(n):
        csp.add_variable(var1, domain=[(var1, i) for i in range(n)])
    for var1 in range(n):
        for var2 in range(var1+1, n):
            csp.add_binary_factor(var1, var2,
                                  lambda (x1, y1), (x2, y2): (y1-y2)*(y1-y2-x1+x2)*(y1-y2+x1-x2) != 0)
    # END_YOUR_CODE
    return csp
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            # BEGIN_YOUR_CODE (our solution is 7 lines of code, but don't worry if you deviate from this)
            priority, mcv_var = 999999, None
            for var in self.csp.variables:
                if var not in assignment:
                    var_priority = sum([self.get_delta_weight(assignment, var, val) for val in self.domains[var]])
                    if var_priority < priority:
                        priority, mcv_var = var_priority, var
            return mcv_var
            # END_YOUR_CODE
    ...
    def arc_consistency_check(self, var):
        ...
        while len(check_queue) > 0:
            ...
            for var2 in self.csp.get_neighbor_vars(cur_var):
                to_add = False
                for var2_val in list(self.domains[var2]):
                    # without the list() above, there will be some fucking BUG!!!
                    if sum([self.csp.binaryFactors[var2][cur_var][var2_val][cur_var_val]
                            for cur_var_val in self.domains[cur_var]]) < 0.5:
                        self.domains[var2].remove(var2_val)
                        to_add = True
                if to_add:
                    check_queue.append(var2)
        # END_YOUR_CODE
